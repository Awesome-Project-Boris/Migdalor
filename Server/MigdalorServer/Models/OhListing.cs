...
namespace MigdalorServer.Models
{
    ...
    public partial class OhListing
    {
        ...
        )
        {
            ...
            if (listingDto.SellerId == Guid.Empty)
            ...
            {
                var pictures = await dbContext
                    .OhPictures.Where(p =>
                        pictureIdsToLink.Contains(p.PicId) && p.ListingId == null
                    )
                    .ToListAsync();
                ...
                {
                    Console.WriteLine(
                        $"WARNING: Could not find/link all provided picture IDs for Listing {newListing.ListingId}."
                    );
                    if (
                        listingDto.MainPicId.HasValue
        ...
        public static async Task<ListingDetail?> GetListingDetailsByIdAsync(
            int listingId,
        ...
        )
        {
            ...
            var listing = await context
                .OhListings.Where(l => l.ListingId == listingId)
            ...
            var pictures = await context
                .OhPictures.Where(p => p.ListingId == listingId) // Filter pictures by the ListingID foreign key
            ...
            var dto = new ListingDetail
            {
                ...
                Date = listing.Date,
                IsActive = listing.IsActive!.Value,
        ...
        )
        {
            ...
            var existingListing = await dbContext.OhListings.FirstOrDefaultAsync(l =>
            ...
            if (updateDto.MainPicId.HasValue)
                newPictureIdsToLink.Add(updateDto.MainPicId.Value);
            if (updateDto.ExtraPicId.HasValue)
                newPictureIdsToLink.Add(updateDto.ExtraPicId.Value);
            ...
            if (newPictureIdsToLink.Any())
            {
                ...
                var picturesToLink = await dbContext
                    .OhPictures.Where(p =>
                        newPictureIdsToLink.Contains(p.PicId) && p.ListingId == null
                    ) // Ensure we only link unlinked pics
                    .ToListAsync();
                ...
                if (picturesToLink.Count != newPictureIdsToLink.Count)
                {
                    ...
                    Console.WriteLine(
                        $"WARNING: Could not find all expected unlinked pictures to link for Listing {listingId}. Provided IDs: {string.Join(",", newPictureIdsToLink)}"
                    );
            ...
            try
            {
                ...
                Console.WriteLine(
                    $"Successfully updated Listing ID: {listingId} and linked pictures."
                );
            ...
            {
                Console.WriteLine(
                    $"ERROR saving updated listing (ID: {listingId}) or linking pictures: {ex.Message} | Inner: {ex.InnerException?.Message}"
                );
                throw new InvalidOperationException(
                    "Failed to save listing updates to the database.",
                    ex
                );
            }
        }
        ...
        ) // Use contentRootPath from IWebHostEnvironment
        {
            if (dbContext == null)
                throw new ArgumentNullException(nameof(dbContext));
            if (string.IsNullOrEmpty(contentRootPath))
                throw new ArgumentNullException(
                    ...
                    "Content root path must be provided for file deletion."
                );

            // Step 1: Fetch the listing itself (WITHOUT including pictures here)
            var listingToDelete = await dbContext.OhListings.FirstOrDefaultAsync(l =>
                l.ListingId == listingId
            );
            ...
            var picturesToDelete = await dbContext
            ...
            {
                Console.WriteLine(
                    $"Found {picturesToDelete.Count} pictures associated with listing {listingId} to delete."
                );
                foreach (var pic in picturesToDelete)
                {
                    bool picFileDeleted = await OhPicture.DeletePictureFileAsync(
                        pic,
                        contentRootPath
                    );
                    if (!picFileDeleted)
                    {
                        Console.WriteLine(
                            $"Warning: Failed to delete picture FILE for ID {pic.PicId} associated with listing {listingId}. Continuing to remove DB record."
                        );
                    }
            ...
            try
            {
                await dbContext.SaveChangesAsync();
                Console.WriteLine(
                    $"Successfully deleted listing ID: {listingId} and associated picture records."
                );
            ...
            {
                Console.WriteLine(
                    $"ERROR deleting listing (ID: {listingId}) during SaveChanges: {ex.Message} | Inner: {ex.InnerException?.Message}"
                );
                // Consider specific error handling/logging
                throw new InvalidOperationException(
                    "Failed to save deletions to the database.",
                    ex
                );
            }
        }

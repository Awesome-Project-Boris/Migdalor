...
const Map = () => {
    ...
    const requestLocationPermission = async () => {
        console.log("[Permissions] Requesting location permission (expo-location)...");
    ...
    useEffect(() => {
        ...
        const activateLocation = async () => {
            ...
            if (granted) {
                ...
                console.log("[Effect] Permission granted. Getting initial position (expo-location)...");
                try {
                    let location = await Location.getCurrentPositionAsync({
                        accuracy: Location.Accuracy.High, // Request high accuracy
                    });
                    console.log("[Effect] getCurrentPositionAsync SUCCESS:", JSON.stringify(location.coords, null, 2));
                    setCurrentUserLocation(location.coords); // Update state with coords object
                    // Initial boundary check
                    const userCoordsForCheck = [location.coords.longitude, location.coords.latitude];
                    ...
                    Alert.alert("Error Getting Location", `Could not fetch initial location: ${error.message}`);
                }
                ...
                try {
                    ...
                    locationSubscription = await Location.watchPositionAsync(
                        ...
                        (location) => { // This callback receives location updates
                            setCurrentUserLocation(location.coords); // Update state with coords object
                    ...
                    Alert.alert("Error Watching Location", `Could not start location updates: ${error.message}`);
                }
        ...
        // --- Cleanup Function ---
        return () => {
            if (watchId.current) { // Check if subscription exists
                console.log("[Effect] Map component unmounting. Removing location subscription.");
            ...
            >
                <View style={styles.modalContainer}>
                    ...
                    <View style={styles.modalView}>
                        ...
                        <FlipButton style={styles.modalButton}  onPress={() => {

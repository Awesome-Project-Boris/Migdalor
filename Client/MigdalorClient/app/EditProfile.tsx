...
//export default function EditProfile( { visible, onClose, onSubmit}: {visible: boolean; onClose: () => void; onSubmit: (data:any) => void; }) {
export default function EditProfile( { visible, onSubmit}: {visible: boolean; onSubmit: (data:any) => void; }) {
  ...
  const inputRefs: { [key: string]: React.RefObject<TextInput> } = {
    partner: useRef<TextInput>(null),
    apartmentNumber: useRef<TextInput>(null),
    mobilePhone: useRef<TextInput>(null),
    email: useRef<TextInput>(null),
    arrivalYear: useRef<TextInput>(null),
    origin: useRef<TextInput>(null),
    profession: useRef<TextInput>(null),
    interests: useRef<TextInput>(null),
    aboutMe: useRef<TextInput>(null),
  };
  ...
  function validateField(name: string, value: string): string | null {
    ...
    switch (name) {
      // case "partner":
      //   return value.trim().length === 0 ? "Partner name is required" : null;
  
      case "apartmentNumber":
        return value === "" || /^[0-9]+$/.test(value) ? null : t("EditProfileScreen_errorMessageApartmentNumber");
  
      case "mobilePhone":
        return value === "" || /^0\d{9}$/.test(value) ? null : t("EditProfileScreen_errorMessageMobilePhone");
  
      case "email":
        return value === "" || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) ? null : t("EditProfileScreen_errorMessageEmail");
  
      case "arrivalYear":
        return value === "" || /^\d{4}$/.test(value) ? null : t("EditProfileScreen_errorMessageArrivalYear");
  
      // case "origin":
      //   return value.trim().length === 0 ? t("EditProfileScreen_errorMessageOrigin") : null;
  
      // case "profession":
      //   return value.trim().length === 0 ? t("EditProfileScreen_errorMessageProfession") : null;
  
      // case "interests":
      //   return value.length > 200 ? t("EditProfileScreen_errorMessageInterests") : null;

      // case "aboutMe":
      //   return value.length > 300 ? t("EditProfileScreen_errorMessageAboutMe") : null;
  ...
  const handleSave = () => {
    ...
    // if (onSubmit) {
    //   onSubmit(form); // !! Pass the updated data to the parent component
    // }
    ...
    <View style={styles.wrapper}>
      <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.editableContainer}>
            ...
            />
            {formErrors.partner && <Text style={styles.errorText}>{formErrors.partner}</Text>}
            ...
            />
            {formErrors.apartmentNumber && <Text style={styles.errorText}>{formErrors.apartmentNumber}</Text>}
            ...
            />
            {formErrors.mobilePhone && <Text style={styles.errorText}>{formErrors.mobilePhone}</Text>}
            ...
            />
            {formErrors.arrivalYear && <Text style={styles.errorText}>{formErrors.arrivalYear}</Text>}
            ...
            />
            {formErrors.aboutMe && <Text style={styles.errorText}>{formErrors.aboutMe}</Text>}
          ...
          <FlipButton onPress={handleCancel} bgColor="white" textColor="black" style={styles.cancelButton}>
            <Text style={styles.buttonLabel}>{t("EditProfileScreen_cancelButton")}</Text>
          </FlipButton>

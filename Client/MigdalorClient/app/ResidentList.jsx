...
export default function ResidentList() {
  ...
  const [activeSearchCriteria, setActiveSearchCriteria] = useState({ query: "", type: "name" }); // Active filter criteria
  ...
  const filteredUsers = useMemo(() => {
    ...
    const filtered = allUsers.filter(user => {
        if (type === "name") {
            ...
            return Array.isArray(user._hobbies_server_only) &&
                   user._hobbies_server_only.some(hobby => hobby?.toLowerCase().includes(lowerCaseQuery));
        }
        return true;
    });
  ...
  const renderUserItem = useCallback(({ item }) => <UserProfileCard data={item} />, []);
  ...
  const handleSearchPress = () => {
   ...
   const CustomEmptyComponent = useMemo(() => {
       if(isLoading && allUsers.length === 0) return <ActivityIndicator size="large" color="#0000ff" style={{marginTop: 50}}/>;
       if(error) return <Text style={styles.errorText}>Error: {error}</Text>;
       if (activeSearchCriteria.query && filteredUsers.length === 0) return <Text style={styles.infoText}>{t("ResidentSearchScreen_noMatchMessage")}</Text>;
       if (!activeSearchCriteria.query && allUsers.length === 0) return <Text style={styles.infoText}>{t("ResidentSearchScreen_noUsersMessage")}</Text>;
       return null; // List has items
   }, [isLoading, error, activeSearchCriteria.query, allUsers, filteredUsers, t]);
    ...
    <View style={styles.container}>
      ...
      >
         <TouchableOpacity onPress={toggleSearchType} style={styles.searchTypeButton}>
             <Text style={styles.searchTypeText}>{t("ResidentSearchScreen_searchByLabel")} <Text style={{ fontWeight: "bold" }}>{searchType === "name" ? t("ResidentSearchScreen_searchByName") : t("ResidentSearchScreen_searchByHobby")}</Text></Text>
         </TouchableOpacity>
         <FloatingLabelInput
         ...
         <FlipButton onPress={handleSearchPress} style={styles.searchSubmitButton} bgColor="#007bff" textColor="#fff">
...
const styles = StyleSheet.create({
   container: { flex: 1, backgroundColor: "#f7f7f7", width: "100%", alignItems: "center", paddingBottom: 10 },
   mainTitle: { fontSize: 32, fontWeight: "bold", textAlign: "center", marginTop: 70, marginBottom: 10, color: "#111" },
   accordionContainer: { width: SCREEN_WIDTH * 0.9, marginVertical: 10, backgroundColor: "#fff", borderRadius: 8, borderWidth: 1, borderColor: "#ddd", overflow: "hidden" },
   searchTypeButton: { paddingVertical: 10, marginBottom: 15, alignItems: "center", backgroundColor: "#e7e7e7", borderRadius: 6, marginHorizontal: 15 },
   ...
   searchSubmitButton: { paddingVertical: 12, marginHorizontal: 15, marginBottom: 15 },
   buttonContent: { flexDirection: "row", alignItems: "center", justifyContent: "center" },
   buttonIcon: { marginRight: 8 },
   searchButtonText: { fontSize: 16, fontWeight: "bold", color: "white" },
   listContainerStyle: { paddingHorizontal: 0, width: '100%', alignItems: 'center' }, // Let PaginatedListDisplay handle internal list width/padding maybe
   infoText: { fontSize: 18, color: "#666", textAlign: "center", marginTop: 50, paddingHorizontal: 20 },
   errorText: { fontSize: 18, color: "red", textAlign: "center", marginTop: 50, paddingHorizontal: 20 },
});
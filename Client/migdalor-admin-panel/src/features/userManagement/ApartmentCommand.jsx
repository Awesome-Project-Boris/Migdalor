...
const ApartmentCommand = ({
  ...
  onSelectApartment,
  label = "מספר דירה",
  placeholder = "בחר/י או הקלד/י מספר דירה...",
}) => {
  ...
  useEffect(() => {
    // If there's a selected apartment, initialize the search with it
  ...
  const handleInputChange = (e) => {
    ...
    setSearch(value);
    onSelectApartment(value); // Update parent state on every keystroke
  };
      ...
      {open && (
        <div className="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-md border">
          <ul className="max-h-60 overflow-y-auto">
            {filteredApartments.length === 0 && search.length > 0 ? (
              <li
